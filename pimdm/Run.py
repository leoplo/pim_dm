...
def main():
    ...
    if args.list_instances:
        multicast_table_id_list = glob.glob(os.path.join(PROCESS_DIRECTORY, '*'))
        t = PrettyTable(['Instance PID', 'Multicast VRF', 'Unicast VRF'])

        for multicast_table_id in multicast_table_id_list:
...
def start(conf_file_path=None):
    ...
    try:
        ...
        multicast_table_id = str(pim_globals.MULTICAST_TABLE_ID)
